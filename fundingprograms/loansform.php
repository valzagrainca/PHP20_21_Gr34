...
if(!empty($Name)||!empty($Surname)||!empty($Email)||!empty($Number)||!empty($Goal)){
  ...
  if(mysqli_connect_error()){
    ...
    $SELECT="SELECT Email From loans Where Email= ? Limit 1";
    $INSERT="INSERT Into loans (Emri,Mbiemri,Email,Phone,Goal) values(?,?,?,?,?)";
    $firstdayofmonth=date('Y-01-d');
    $yesterday=date("Y-m-d", strtotime('-1 day'));
    $today=date('Y-m-d');
    $date = date("Y-m-d", strtotime('-1 month'));
    $DELETE="DELETE From loans Where register_date BETWEEN '". $date ."' AND '". $yesterday ."'";
    $result = mysqli_query($conn,"SELECT register_date From loans Where register_date BETWEEN '". $date ."' AND '". $yesterday ."'");
    $random_name="";
    $query_random="SELECT Id,Emri,Mbiemri,Email From loans ORDER BY RAND() LIMIT 1";
